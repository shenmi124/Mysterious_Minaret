...
// data = a function returning the content (actually HTML)
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			v-bind:style="[(hasUpgrade(layer, data) ? {'background-color': '#FFFFFF'} : canAffordUpgrade(layer, data) ? {'background-color': '#886f9b'} : {'background-color': '#494949'}), tmp[layer].upgrades[data].style]">
			...
			<span v-else>
				...
				<br><br>需求: {{ formatWhole(tmp[layer].upgrades[data].cost) }} {{(tmp[layer].upgrades[data].currencyDisplayName ? tmp[layer].upgrades[data].currencyDisplayName : tmp[layer].resource)}}
			</span>
	...
	Vue.component('clickable', {
		...
		<button
			...
			v-bind:style="[
			player.data.themes_light==true ? {'background-color': '#FFFFFF'} : {'background-color': '#000000'},
			player.data.themes_light==true ? tmp[layer].clickables[data].canClick ? {'border': '2px solid #000000'} : {'border': '2px solid #aaaaaa'} : tmp[layer].clickables[data].canClick ? {'border': '2px solid #FFFFFF'} : {'border': '2px solid #666666'},
			player.data.themes_light==true ? tmp[layer].clickables[data].canClick ? {'color': '#000000'} : {'color': '#aaaaaa'} : tmp[layer].clickables[data].canClick ? {'color': '#FFFFFF'} : {'color': '#666666'},
			{'border-radius': '17px'},
